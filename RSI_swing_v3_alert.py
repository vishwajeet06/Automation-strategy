...
async def fetch_stock_data(stock, tsl, semaphore):
    async with semaphore:  # Limit concurrent requests
        try:
            ...
            # Fetch daily and intraday data asynchronously
            ...
            # Calculate RSI
            rsi_series = talib.RSI(pd.Series(chart_daily["close"]), 7)

            # Store yesterday's RSI and current RSI
            if len(rsi_series) > 1:
                rsi_yesterday = round(rsi_series.iloc[-2], 2)
                rsi_today = round(rsi_series.iloc[-1], 2)
                rsi_cache[stock] = {"yesterday": rsi_yesterday, "current": rsi_today}

                print(f"Updated RSI for {stock}: Yesterday = {rsi_yesterday}, Current = {rsi_today}")

                # Send alert if yesterday RSI < 20 and current RSI is between 20-24
                if rsi_yesterday < 20 and 20 <= rsi_today <= 24:
                    message = f"RSI Alert for {stock} 📉\nYesterday RSI: {rsi_yesterday}\nCurrent RSI: {rsi_today}"
                    await loop.run_in_executor(None, tsl.send_telegram_alert, message, TELEGRAM_CHAT_ID, TELEGRAM_BOT_TOKEN)
...
async def update_rsi_values():
    ...
    semaphore = asyncio.Semaphore(10)  # Limit to 10 concurrent requests (OHLC rate limit)
    ...
    while True:
        ...
        for i in range(0, len(stocks), 10):
            ...
            await asyncio.sleep(1)  # Wait 1 second between batches to respect rate limits
...
def update_stock_data(response):
    ...
    for stock, details in stock_data.items():
        if str(details["security_token"]) == security_id:
            ...
            rsi_values = rsi_cache.get(stock, {"yesterday": "N/A", "current": "N/A"})
            details["rsi_yesterday"] = rsi_values["yesterday"]
            details["rsi_current"] = rsi_values["current"]
