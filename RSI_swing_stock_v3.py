...
async def fetch_stock_data(stock, tsl, semaphore):
    async with semaphore:  # Limit concurrent requests
        try:
            ...
            if(rsi_cache[stock]>=20 and rsi_cache[stock]<=25):
                tsl.send_telegram_alert(message = rsi_cache[stock],receiver_chat_id=TELEGRAM_CHAT_ID, bot_token=TELEGRAM_BOT_TOKEN)
        except Exception as e:
